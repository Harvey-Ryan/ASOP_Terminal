...
// Admin manually links an RSI handle to a Discord member's app account.

rsiRouter.post('/:guildId/rsi/assign', requireAuth, async (req, res) => {
  const { guildId } = req.params as { guildId: string };
...
// Admin clears a manually (or user-) linked RSI handle.

rsiRouter.delete('/:guildId/rsi/assign/:discordId', requireAuth, async (req, res) => {
