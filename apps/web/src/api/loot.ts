...
export const lootApi = {
  ...
  getPlayers: (guildId: string, eventId?: string) =>
    api.get<ApiResponse<{ userId: string; username: string }[]>>(
      `/guilds/${guildId}/dkp/players${eventId ? `?eventId=${encodeURIComponent(eventId)}` : ''}`,
    ).then((r) => r.data ?? []),
