...
function fmtDiffLabel(raw: string): string {
...
import type { MissionDifficulty, MissionStanding } from '@/lib/gameData';
...
function DifficultyBlock({ diff, time, standing, illegal }: {
  ...
  return (
    <div className="space-y-2">
      {/* Meta row: time + profile + standing + illegal */}
      ...
      {expanded && (
        <div className="px-3 py-2.5 space-y-2.5">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {contract.difficulty && (
              <span className={cn('text-[10px] border rounded px-1.5 py-0.5 font-semibold', diffCls(contract.difficulty.riskOfLoss))}>
                Risk {contract.difficulty.riskOfLoss}/7
              </span>
            )}
            {contract.illegal && (
              <span className="text-[10px] bg-red-500/10 text-red-500 border border-red-500/20 rounded px-1.5 py-0.5 font-semibold">Illegal</span>
            )}
            {contract.minStanding && contract.minStanding.minReputation > 0 && (
              <span className="text-[10px] bg-primary/10 text-primary rounded px-1.5 py-0.5">{contract.minStanding.name}</span>
            )}
          </div>
...
export function CraftingCalculatorPage() {
  ...
  const browseList = useMemo(
    () => (typeFilter || inputValue.length > 1)
      ? searchBlueprints(inputValue, typeFilter || undefined).slice(0, 25)
      : [],
    [inputValue, typeFilter],
  );

  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    if (e.key === 'Enter' && browseList.length > 0) {
      e.preventDefault();
      navigate(`/dashboard/servers/${guildId}/crafting-calculator/${browseList[0].uuid}`);
    }
  }
        ...
        {browseList.length > 0 ? (
          <div className="divide-y divide-border">
            {browseList.map(bp => (
              <button
                ...
                className="w-full flex items-center gap-3 px-6 py-2.5 text-left hover:bg-accent transition-colors"
              >
                <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-col items-center justify-center h-full text-center px-6 gap-3">
            ...
            <p className="text-sm text-muted-foreground">
              Select a type or search above to browse blueprints.
            </p>
          </div>
        )}
      </div>
